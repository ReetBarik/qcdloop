...
int main(int argc, char* argv[]) {
    Kokkos::initialize(argc, argv);
    {
        ...
        // BIN4
        for (size_t i(0); i<batch_size; ++i) {
            m_h(i, 0) = q_to_dd(10); m_h(i, 1) = q_to_dd(10); m_h(i, 2) = q_to_dd(10); m_h(i, 3) = q_to_dd(10);
            p_h(i, 0) = q_to_dd(rs(low,up));
            p_h(i, 1) = q_to_dd(rs(low,up));
            p_h(i, 2) = q_to_dd(rs(low,up));
            p_h(i, 3) = q_to_dd(rs(low,up));
            p_h(i, 4) = q_to_dd(r(low,up));
            p_h(i, 5) = q_to_dd(r(low,up));
        }
