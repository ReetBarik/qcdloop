...
int main(int argc, char* argv[]) {
    Kokkos::initialize(argc, argv);
    {
        ...
        // BIN1
        for (size_t i(0); i<batch_size; ++i) {
            m_h(i, 0) = static_cast<fp128_t>(10); m_h(i, 1) = static_cast<fp128_t>(0.); m_h(i, 2) = static_cast<fp128_t>(0.); m_h(i, 3) = static_cast<fp128_t>(0.);
        ...
        tt.start();
        Kokkos::parallel_for("Box Integral BIN1", policy, KOKKOS_LAMBDA(const int& i) {
        ...
        if (mode == 0) {
            std::cout << "BIN1," << batch_size << "," << elapsed << std::endl;
        } else if (mode == 1) {
            for (size_t i = 0; i < batch_size; ++i) {
