...
int main(int argc, char* argv[]) {
    Kokkos::initialize(argc, argv);
    {
        ...
        // B14
        for (size_t i(0); i<batch_size; ++i) {
            m_h(i, 0) = q_to_dd(0.); m_h(i, 1) = q_to_dd(m22); m_h(i, 2) = q_to_dd(0.); m_h(i, 3) = q_to_dd(m42);
            p_h(i, 0) = q_to_dd(m22); p_h(i, 1) = q_to_dd(m22);
            p_h(i, 2) = q_to_dd(m42); p_h(i, 3) = q_to_dd(m42);
