...
int main(int argc, char* argv[]) {
    Kokkos::initialize(argc, argv);
    {
        ...
        // BIN4
        for (size_t i(0); i<batch_size; ++i) {
            m_h(i, 0) = static_cast<fp128_t>(10); m_h(i, 1) = static_cast<fp128_t>(10); m_h(i, 2) = static_cast<fp128_t>(10); m_h(i, 3) = static_cast<fp128_t>(10);
            p_h(i, 0) = static_cast<fp128_t>(rs(low,up));
            p_h(i, 1) = static_cast<fp128_t>(rs(low,up));
            p_h(i, 2) = static_cast<fp128_t>(rs(low,up));
            p_h(i, 3) = static_cast<fp128_t>(rs(low,up));
            p_h(i, 4) = static_cast<fp128_t>(r(low,up));
            p_h(i, 5) = static_cast<fp128_t>(r(low,up));
        }
        Kokkos::deep_copy(mu2_d, mu2_h);
        Kokkos::deep_copy(m_d, m_h);
        Kokkos::deep_copy(p_d, p_h);
        tt.start();
        Kokkos::parallel_for("Box Integral BIN4", policy, KOKKOS_LAMBDA(const int& i) {
        ...
        if (mode == 0) {
            std::cout << "BIN4," << batch_size << "," << elapsed << std::endl;
        } else if (mode == 1) {
            for (size_t i = 0; i < batch_size; ++i) {
